-- Complete Company Update Script (Run this single script)
-- Canhav - Adds new columns and updates company data

-- Step 1: Create backup table (drop existing one first)
DROP TABLE IF EXISTS vasp_companies_backup;
...
-- Step 4: Add unique constraint for exchange_services_details if it doesn't exist
DO $$ 
BEGIN
    IF NOT EXISTS (
...
-- Step 5: Update existing companies or insert new ones
...
-- Step 6: Update sector-specific details (exchange services)
